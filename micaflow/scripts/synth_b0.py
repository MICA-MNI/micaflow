...
def cleanup_ants_temp_files():
    """
    Clean up temporary files generated by ANTs registration.
    
    ANTs registration creates various temporary files during processing including
    transformation matrices, warp fields, and intermediate images. This function
    removes these files to prevent clutter and save disk space.
    
    Common temporary file patterns cleaned:
    - tmp*.nii.gz: Temporary NIfTI images
    - tmp*.mat: Temporary transformation matrices
    - *Warp.nii.gz: Forward warp fields
    - *InverseWarp.nii.gz: Inverse warp fields
    - *Affine.txt: Affine transformation text files
    - *GenericAffine.mat: Generic affine matrices
    - ants_*: ANTs process files
    
    Notes
    -----
    - Silently ignores files that cannot be removed
    - Prints warning if file removal fails
    - Reports total number of files cleaned
    - Safe to call multiple times
    - Does not remove user-specified output files
    
    Examples
    --------
    >>> # Clean up after registration
    >>> cleanup_ants_temp_files()
    Cleaned up 15 temporary ANTs files
    """
    # Common patterns for ANTs temporary files
    temp_patterns = [
        "tmp*.nii.gz",
        "tmp*.mat", 
        "*Warp.nii.gz",
    ...
    for pattern in temp_patterns:
        for file_path in glob.glob(pattern):
            try:
                ...
                print(f"{YELLOW}Warning: Could not remove {file_path}: {e}{RESET}")
    
    if cleaned_files > 0:
        print(f"{GREEN}Cleaned up {cleaned_files} temporary ANTs files{RESET}")
...
def main():
    ...
    try:
        ...
        if args.cpu:
            ...
            print(f"  {YELLOW}Warning: Processing will be slow (30-90 min){RESET}")
        else:
            if torch.cuda.is_available():
                device = torch.device("cuda")
                print(f"  Device: GPU - {torch.cuda.get_device_name(0)}")
                print(f"  {GREEN}GPU acceleration enabled (5-15 min){RESET}")
            else:
                device = torch.device("cpu")
        ...
        synthetic_b0_in_T1space = ants.registration(
            fixed=b0_img,
            moving=synthetic_b0,
        ...
        print()
        if 'fwdtransforms' in synthetic_b0_in_T1space:
            for transform_file in synthetic_b0_in_T1space['fwdtransforms']:
        ...
        if 'invtransforms' in synthetic_b0_in_T1space:
            for transform_file in synthetic_b0_in_T1space['invtransforms']:
                if os.path.exists(transform_file):
                    try:
                        os.remove(transform_file)
                    except Exception as e:
