...
def check_paths(
    ...
    if SESSION == "":
        print_warning(
            "Session not provided, if the dataset does not have sessions, please set the SESSION argument to 'None'."
        )
    if SESSION == "None":
        print_note("Session is set to None.")
        SESSION = None
    if GPU == "":
        print_note("GPU not provided, defaulting to CPU.")
        GPU = False
    else:
    ...
    if DWI_FILE != "":
        ...
        if BVAL_FILE == "":
            print_error(
                "BVAL file not provided. BVAL file is required for diffusion processing."
            )
            sys.exit(1)
        if BVEC_FILE == "":
            print_error(
                "BVEC file not provided. BVEC file is required for diffusion processing."
            )
            sys.exit(1)
        if INVERSE_DWI_FILE == "":
            print_error(
    ...
    if DATA_DIRECTORY != "":
        ...
        print_note(
            "Data directory not provided, file paths are assumed to be absolute paths to the relevant files."
        )
        if RUN_FLAIR:
    ...
    if OUT_DIR != "":
        ...
        print_note(
            "Output directory not provided, output will be saved in the current working directory."
        )
