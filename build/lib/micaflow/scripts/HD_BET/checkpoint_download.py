...
from paths import folder_with_parameter_files, ZENODO_DOWNLOAD_URL
...
def maybe_download_parameters():
    if not isfile(
        join(folder_with_parameter_files, "fold_all", "checkpoint_final.pth")
    ):
        maybe_mkdir_p(folder_with_parameter_files)
        fname = download_file(
            ZENODO_DOWNLOAD_URL,
            join(folder_with_parameter_files, os.pardir, "tmp_download.zip"),
        )
